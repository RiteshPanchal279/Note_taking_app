...
router.post('/signup', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!email || !name || !dob) return res.status(400).json({ error: 'All fields are required',success:false });
...
router.post('/signup-verify', asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json({ token, user: { name: user.name, dob: user.dob, email: user.email },success:true });
}));
...
router.post('/login', asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = await User.findOne({ email });
  if (!user) return res.status(404).json({ error: 'User not found',success:true });
